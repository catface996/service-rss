package com.catface.rss.service.resource.util;
...
public class ResourceUtil {
     ...
     * @param signatureDTO 临时访问阿里云授权签名
    ...
    public static boolean uploadFile(InputStream inputStream, String fileName, String contentType,
                                     SignatureDTO signatureDTO, int timeout) throws IOException {
        RequestConfig requestConfig = RequestConfig.custom()
            ...
            .build();
        HttpPost post = new HttpPost(signatureDTO.getHost());
        ...
        try (CloseableHttpClient client = HttpClients.createDefault()) {
            ...
            StringBody accessKeyId = new StringBody(signatureDTO.getAccessId(),
                ContentType.MULTIPART_FORM_DATA);
            builder.addPart(OSS_ACCESS_KEY_ID, accessKeyId);

            StringBody policy = new StringBody(signatureDTO.getPolicy(),
                ContentType.MULTIPART_FORM_DATA);
            builder.addPart(POLICY, policy);

            String callback = signatureDTO.getCallback();
            ...
            StringBody signature = new StringBody(signatureDTO.getSignature(),
                ContentType.MULTIPART_FORM_DATA);
            builder.addPart(SIGNATURE, signature);

            // 兼容key中的中文名称
            StringBody key = new StringBody(signatureDTO.getKey(),
